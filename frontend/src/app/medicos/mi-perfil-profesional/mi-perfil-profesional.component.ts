...
@Component({
  selector: 'app-mi-perfil-profesional',
  templateUrl: './mi-perfil-profesional.component.html',
  styleUrls: ['./mi-perfil-profesional.component.scss']
})
export class MiPerfilProfesionalComponent implements OnInit {
  ...
  /**
   * Obtiene el RUT del médico autenticado
   */
  get rutMedico(): string {
    if (this.authService.medico && this.authService.medico.rut) {
      return this.authService.medico.rut;
    }
    return '';
  }

  /**
   * Carga los datos del médico autenticado
   */
  cargarDatosMedico(): void {
    // Verificar que el médico esté autenticado
    if (!this.authService.medico || !this.authService.medico.rut) {
      console.error('No se encontró médico autenticado');
      Swal.fire('Error', 'No se pudo identificar su cuenta de médico', 'error');
    ...
    // Cargar perfil completo usando el RUT del médico autenticado
    this.medicoService.obtenerPerfilMedico(this.rutMedico).subscribe({
      ...
      error: (error) => {
        ...
        Swal.fire('Error', 'No se pudo cargar su información profesional', 'error');
        this.cargando = false;
      }
    });
  }
   ...
   */
  volver(): void {
    this.router.navigate(['/medicos/gestionar-historiales']);
  }
